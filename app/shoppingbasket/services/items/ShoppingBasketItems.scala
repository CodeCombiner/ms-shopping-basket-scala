...
case class ProductDisplay(id: String, name: String, description: String, price: BigDecimal, currency: String, stock: Long)

object ProductDisplay {

  def apply(product: Product): ProductDisplay =
    ProductDisplay(product.id, product.name, product.description, product.price, product.currency, product.stock)
}
