package shoppingbasket.services.akka
...
import shoppingbasket.services.items.{ServiceErrors, ShoppingBasketCreate, ShoppingBasketCreateItem, ShoppingBasketCreateProductItem}
  ...
  "ShoppingBasketActor#handleShoppingBasketCreateRequest" should {
    "do nothing if the given sequence is empty" in { implicit ee: ExecutionEnv =>
      ...
      val responseAsFuture = (actorRef ? ShoppingBasketCreate(
        Seq(ShoppingBasketCreateItem(ShoppingBasketCreateProductItem(products.head.id), 0))
    ...
    "create a new shopping basket if a valid product is given" in { implicit ee: ExecutionEnv =>
      ...
      val responseAsFuture = (actorRef ? ShoppingBasketCreate(
        Seq(ShoppingBasketCreateItem(ShoppingBasketCreateProductItem(products.head.id), 2))
      )).mapTo[Either[ServiceErrors, String]]
